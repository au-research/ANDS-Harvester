...
class test_get_harvester(unittest.TestCase):
    ...
    def test_XML_get(self, mockGetData):
        f = io.open(myconfig.run_dir + 'tests/resources/test_source/get/get_xml.xml', mode="r")
        data = f.read()
        f.close()
    ...
    def test_JSON_get(self, mockGetData):
        f = io.open(myconfig.run_dir + 'tests/resources/test_source/get/get_json.json', mode="r")
        data = f.read()
        f.close()
        mockGetData.return_value = data
        harvestInfo = {}
