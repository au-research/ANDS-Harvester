...
class ARCQUERYHarvester(Harvester):
    ...
    def getHarvestData(self):
        """
        gets a set of 400  ARC SOLR results in JSON format
        :return:
        """
        if self.stopped:
            return
        ...
        try:
            ...
            self.logger.logMessage(
                "ARC QUERY (getHarvestData), getting data url:%s" %(request_url),
                "DEBUG")
            ...
            if self.numberOfRecordsReturned == 0 or (self.harvestInfo['mode'] == 'TEST' and self.recordCount >= myconfig.test_limit):
                ...
                self.logger.logMessage("ERROR RECEIVING  ARC QUERY DATA, %s" % str(repr(e)), "ERROR")
        del getRequest
    ...
    def getRequestUrl(self):
        ...
        :return url:
        """
        urlParams = 'page[number] = ' + self.pageCount + 'page[size] =' + self.rows
    ...
    def getRecordCount(self):
        ...
        try:
            ...
            self.logger.logMessage("Can we log data?" + json.dumps(data))
