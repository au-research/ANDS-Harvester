    ...
    def getHarvestData(self):
        ...
        while retryCount > 0:
            try:
                ...
                if retryCount <= 0:
                    self.errored = True
                    self.handleExceptions("ERROR RECEIVING CKANQUERY DATA, retry left:%s, error: %s, url:%s"
                                %(str(retryCount), str(repr(e)), request_url))
                else:
                    self.logger.logMessage("ERROR RECEIVING CKANQUERY DATA, retry left :%s, error: %s, url:%s"
